<template>
    <div class="flex h-screen w-screen font-sans antialiased text-gray-900 bg-gray-100 dark:bg-gray-900 dark:text-gray-100">
        <!-- Sidebar -->
        <SettingsSidebar
            :sections="sections"
            :active-section="activeSection"
            @update:activeSection="setActiveSection"
            class="flex-shrink-0 w-64 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-y-auto scrollbar-thin scrollbar-track-bg-light-tone scrollbar-thumb-bg-light-tone-panel hover:scrollbar-thumb-primary dark:scrollbar-track-bg-dark-tone dark:scrollbar-thumb-bg-dark-tone-panel dark:hover:scrollbar-thumb-primary active:scrollbar-thumb-secondary"
        />

        <!-- Main Content Area -->
        <div class="flex-1 flex flex-col overflow-hidden">
            <!-- Top Control Bar -->
            <div :class="['sticky top-0 z-20 flex items-center justify-between p-3 gap-3 shadow-md panels-color', isLoading ? 'opacity-50 pointer-events-none':'']">
                <div class="flex items-center gap-4 flex-1">
                    <!-- Save/Cancel Confirmation (Appears when settingsChanged is true) -->
                     <div v-if="settingsChanged && !isLoading" class="flex items-center gap-2 text-amber-600 dark:text-amber-400 animate-pulse">
                    ...
                    <div v-if="!settingsChanged && !isLoading" class="flex gap-3 items-center">
                         <button title="Reset configuration to default" class="icon-button hover:text-secondary" @click="reset_configuration()">
                            <i data-feather="refresh-ccw"></i>
                         </button>
                         <button title="Restart program" class="icon-button hover:text-secondary" @click="restart_program()">
                            <i data-feather="power"></i>
                         </button>
                         <button title="Clear uploads folder" class="icon-button hover:text-secondary" @click="clear_uploads()">
                             <i data-feather="trash-2"></i>
                         </button>
                         <button v-if="has_updates" title="Upgrade program" class="icon-button text-green-500 hover:text-secondary relative" @click="update_software()">
                ...
                <!-- Apply/Cancel Buttons (Appears when settingsChanged is true) -->
                <div v-if="settingsChanged && !isLoading" class="flex items-center gap-2">
                     <button class="flex items-center gap-2 px-3 py-1.5 rounded text-white bg-green-600 hover:bg-green-700 duration-150 active:scale-95"
                             title="Apply changes"
                             type="button"
                     ...
                     <button class="flex items-center gap-2 px-3 py-1.5 rounded text-white bg-red-600 hover:bg-red-700 duration-150 active:scale-95"
...
export default {
    ...
    data() {
        return {
            ...
            configFile: null, // Will hold the config object
    ...
    watch: {
        configFile: {
            handler(newVal, oldVal) {
                 // Ensure oldVal isn't null (initial load) and there's an actual change
                if (oldVal !== null && newVal !== null && JSON.stringify(newVal) !== JSON.stringify(oldVal)) {
                    // Check if the change originated from user input vs. a refreshConfig call
                    // Compare against the store's config *before* it might have been updated by refreshConfig
                    // This comparison might be slightly less reliable than in setup script depending on timing,
                    // but it's the closest equivalent.
                    if (JSON.stringify(newVal) !== JSON.stringify(this.$store.state.config)) {
                        console.log("Config changed locally");
                        this.settingsChanged = true;
                    } else {
                        console.log("Config changed due to external refresh or programmatic update matching store");
                        // If the change was due to refresh, settingsChanged should already be false or will be set to false by refreshConfig
                    }
                }
            },
            deep: true // Necessary for watching nested properties
        }
    },
    methods: {
        ...
        handleSettingUpdate(payload) {
            ...
            let current = this.configFile; // Use this.configFile
            try {
                while (keys.length > 1) {
                    const k = keys.shift();
        ...
        async refreshConfig() {
            ...
            try {
                ...
                this.configFile = JSON.parse(JSON.stringify(this.$store.state.config));
        ...
        async applyConfiguration() {
            ...
            try {
                ...
                const res = await axios.post('/apply_settings', { client_id: this.$store.state.client_id, config: this.configFile }, { headers: posts_headers });
                if (res.data.status) {
        ...
        async cancelConfiguration(askRefresh = true) {
            if (askRefresh) {
                ...
                await this.refreshConfig();
                 ...
                 // Just revert local changes to match the store's current state (deep copy)
                this.configFile = JSON.parse(JSON.stringify(this.$store.state.config));
