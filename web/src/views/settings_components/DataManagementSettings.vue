...
<template>
    <div class="space-y-6 p-4 md:p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
        ...
        <section class="space-y-4 p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
            ...
            <!-- Data Lakes List -->
            <div class="space-y-4">
                 <div v-if="!$store.state.config.datalakes || $store.state.config.datalakes.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-4">
        ...
        <section class="space-y-4 p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
            <h3 class="text-lg font-medium text-gray-700 dark:text-gray-300 mb-3">Self-Hosted RAG Servers</h3>
             ...
             <!-- Servers List -->
            <div class="space-y-4">
                 <div v-if="!$store.state.config.rag_local_services || $store.state.config.rag_local_services.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-4">
        ...
        <section class="space-y-4 p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
             ...
             <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <div>
                    ...
                    <select id="rag_vectorizer" required :value="$store.state.config.rag_vectorizer" @change="updateValue('rag_vectorizer', $event.target.value)" class="input-field-sm w-full">
                ...
                <div class="flex items-end pb-1">
                    <div class="flex items-center space-x-2">
                        <ToggleSwitch id="rag_vectorizer_execute_remote_code" :checked="$store.state.config.rag_vectorizer_execute_remote_code" @update:checked="updateBoolean('rag_vectorizer_execute_remote_code', $event)" />
            ...
            <div class="setting-item">
                ...
                <div class="flex-1 flex items-center gap-4">
                    <input id="rag_chunk_size-range" :value="$store.state.config.rag_chunk_size" @input="updateValue('rag_chunk_size', parseInt($event.target.value))" type="range" min="100" max="2000" step="50" class="range-input">
                    <input id="rag_chunk_size-number" :value="$store.state.config.rag_chunk_size" @input="updateValue('rag_chunk_size', parseInt($event.target.value))" type="number" min="100" max="2000" step="50" class="input-field-sm w-24 text-center">
                </div>
            </div>
            ...
            <div class="setting-item">
                <label for="rag_overlap_size" class="setting-label">Overlap Size</label>
                 <div class="flex-1 flex items-center gap-4">
                    <input id="rag_overlap_size-range" :value="$store.state.config.rag_overlap_size" @input="updateValue('rag_overlap_size', parseInt($event.target.value))" type="range" min="0" max="500" step="10" class="range-input">
                    <input id="rag_overlap_size-number" :value="$store.state.config.rag_overlap_size" @input="updateValue('rag_overlap_size', parseInt($event.target.value))" type="number" min="0" max="500" step="10" class="input-field-sm w-24 text-center">
                </div>
            </div>

             <!-- Clean Chunks Toggle -->
            <div class="toggle-item !justify-start gap-4">
                 <ToggleSwitch id="rag_clean_chunks" :checked="$store.state.config.rag_clean_chunks" @update:checked="updateBoolean('rag_clean_chunks', $event)" />
        ...
        <section class="space-y-4 p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
            ...
            <!-- Reformulate Prompt -->
             <div class="toggle-item">
                ...
                <ToggleSwitch id="rag_build_keys_words" :checked="$store.state.config.rag_build_keys_words" @update:checked="updateBoolean('rag_build_keys_words', $event)" />
            </div>
            ...
            <div class="toggle-item">
                ...
                <ToggleSwitch id="rag_put_chunk_informations_into_context" :checked="$store.state.config.rag_put_chunk_informations_into_context" @update:checked="updateBoolean('rag_put_chunk_informations_into_context', $event)" />
            </div>
            ...
            <div class="toggle-item">
                <label for="data_vectorization_save_db" class="toggle-label">
                 ...
                 <ToggleSwitch id="data_vectorization_save_db" :checked="$store.state.config.data_vectorization_save_db" @update:checked="updateBoolean('data_vectorization_save_db', $event)" />
            </div>
        </section>
...
export default {
    ...
    emits: ['update:setting', 'settings-changed'], // Use standard emits option
    data() {
        return {
    ...
    methods: {
        ...
        updateDataLake(index, field, value) {
            ...
            if (currentDatalakes[index]) {
                ...
                this.$emit('update:setting', { key: 'datalakes', value: currentDatalakes });
            } else {
                 console.error("Attempted to update non-existent datalake at index:", index);
            }
            // For single field updates, emitting the specific path might be more efficient if the parent component handles it directly:
            // this.$emit('update:setting', { key: `datalakes[${index}].${field}`, value });
        },
        ...
        updateServer(index, field, value) {
             ...
             if (currentServers[index]) {
                 ...
                 this.$emit('update:setting', { key: 'rag_local_services', value: currentServers });
             } else {
                console.error("Attempted to update non-existent server at index:", index);
             }
            // Or emit specific path:
            // this.$emit('update:setting', { key: `rag_local_services[${index}].${field}`, value });
        },
